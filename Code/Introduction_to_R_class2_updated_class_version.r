...
str(iris)
boxplot(iris$Sepal.Length) # Quick boxplot of your data. Use the $ to choose the variable you're interested in visualising.
...
iris_set_petal_long <- filter(iris_setosa, Petal.Length > 1.5 & Sepal.Length > 3) # filter example.
...
iris_set_petal_long[seq(4,8),c(2,3:4)] # This can be combined with the seq() function we used last week. However, this can look like an overwhelming number of numbers. Thankfully there are means of simplifying the process. For example, we can use head() and tail to preview apects of the data.

head(iris_set_petal_long) # View the first 5 rows
tail(iris_set_petal_long) # View the last 5 rows


iris_set_petal_long %>% slice_min(Sepal.Length, n =3) # A funky little demo of what tidyverse can offer.  'slice_min()' allows us to view the lowest values associated with a variable. Try changing _min to _max and see what happens. We will explain the %>% function later, but for now this of it as a instructing tool for R.
...
# Make sure that your R script and dataset are stored in the same folder.
# From there, go to Session, and Set Working Directory to "To source file location". This will tell R to make sure that any interactions with the computer are focused into the same file as your script. This helps it to find the appropriate data that is needed.

read_csv("2021_election.csv") # The read_csv() function in 'readr' part of the tidyverse. 

# Make sure to assign the dataset with a name - Personally I prefer using short names, as it makes life easier for typing - i.e., it takes less effort to be accurate writing "Elec" compared to writing "2021_election.csv" over and over. If using multiple datasets, make sure the assigned names are meaningful to you, as it can help prevent confusion of having to identify what is what.
...
spec_csv("2021_election.csv") # The spec_csv() function in readr provides information about the data. We can get a similar output by running str(Elec)  
str(Elec)
...
# Finding Specific Data - base R approaches. They can be useful, but genereally speaking the Tidyverse approach is easier to interpret and understand, which in turn helps to reduce the chances of making mistakes.

Elec[,3] # Select column - this is done by inputting the appropriate number on the right hand side of the comma.
Elec[5,] # Select row - this is done by inputting the appropriate number on the left hand side of the comma. 
Elec[15,1] # Select cell - by inputting numbers on both the left and right hand side of the comma, base R will select cells based by the row and column.
...
## Base R - (Requires knowing exactly what row you want to extract)
Row_15 <- Elec[15,]
# Subset is another base R approach, and requires repetition of "Elec" in the command. Intially we tell the function which dataset we want it to look in, then we tell it which variable, and finally what rule we wish to subset by.
Lothian <- subset(Elec, Elec$Region == 'Lothian')
summary(Lothian)
head(Lothian)

#Here is the tidyverse approach. Notice how "Elec" only needs to be written down once. The first stage is setting the dataset. This is followed by the pipe, indicating that we want to take "Elec" on the left, and apply "filter()" on the right to it. Within filter, we can choose the rule by which want to filter the data by. Here we have asked R to filter the dataset by only selecting Regions that are "Lothian". Please note that we can use either "" or '' here. The choice is stylistic. 
...
# This can also be done in through fewer lines of code, but in my opinion it is less intuitive and user friendly. This can create reproduce-ability issues, and make it difficult to interpret what is actually happening in the code. Below is the "base-r" approach to doing what was done above. Please also note that the base-r approach below happens to use the wide format of data (which also adds to the complications of interpreting what is happening).
...
Merged_data <- merge(..., ..., by.x="Constituency", by.y="Constit")
...
# Example plot using the iris dataset
  ggplot(iris, aes(x = Sepal.Length, y = Sepal.Width, color = Species)) + #See what happens here if you remove `, color = Species` from the code. Do you notice a difference in the trend? Data visualisation can be used to highlight nuance in our data.
  geom_point(alpha = .7, position = "jitter") + #alpha allows us to decide the transparency of the points. Setting position to "jitter" moves all the points away the co-ordinates ever so slightly. This combination is useful if you want to demonstrate overlapping points. 
  geom_smooth(method = "lm", se= FALSE) # setting method to "lm" forces R to use a linear model in producing the line of best fit. Setting se to "FALSE" asks R to remove the standard error in the visualisation. Try setting it to "TRUE" and see the difference.
  ...
  linear_model <- lm(Sepal.Length ~  Sepal.Width + Species, data = iris) # linear regression coding. Use the plus sign to add additional variable. Use * to mark interaction effects. 
