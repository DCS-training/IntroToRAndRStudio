...
library(dplyr) #installed with tidyverse
library(tidyr) #installed with tidyverse
```
...
tail(penguins, n = 8) # Lets us view the last 8 rows
...
-   Use `hist()` and `boxplot()` to check the distribution of
    `body_mass_g`.
-   Identify the number of penguins in each Island.
...
sum(is.na(penguins$sex)) # Total number of NAs in a specific column
sum(!complete.cases(penguins)) # total number of rows that contain at least one NA
penguins_clean<- penguins %>% drop_na() # removes all the rows if there is at least one NA
penguins_clean_2<- penguins %>% drop_na(sex) # removes all the rows if there is at least one NA in a specific column
